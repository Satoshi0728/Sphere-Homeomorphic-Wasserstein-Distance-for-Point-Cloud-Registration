...
def sliced_cost(Xs, Xt, Us, p=2, u_weights=None, v_weights=None):
    """
        ...
        p: float
            Power
    """
    batch_size, n_projs, d, k = Us.shape
    ...
    ## Projection on S^1
    ## Projection on plane
    Xps = torch.permute(torch.matmul(torch.permute(Us, (1,0,3,2))[:,None],torch.transpose(Xs,0,1)[:,:, :, None]), (2,0,1,3,4)).reshape(batch_size, n_projs, n, 2)
    Xpt = torch.permute(torch.matmul(torch.permute(Us, (1,0,3,2))[:,None],torch.transpose(Xt,0,1)[:,:, :, None]), (2,0,1,3,4)).reshape(batch_size, n_projs, m, 2)
    ...
    if p==1:
        ...
        w1 = torch.zeros((1), device=Xs.device)
        for i in range(batch_size):
            w1 += torch.mean(binary_search_circle(Xps[i], Xpt[i], p=p, u_weights=u_weights, v_weights=v_weights))
        # w1 = binary_search_circle(Xps, Xpt, p=p, u_weights=u_weights, v_weights=v_weights)
    return w1


def sliced_wasserstein_sphere_fast(Xs, Xt, num_projections, device, u_weights=None, v_weights=None, p=2):
    """
    ...
    Z = torch.randn((batch_size,num_projections,d,2), device=device)
    U, _ = torch.linalg.qr(Z)
    return sliced_cost(Xs, Xt, U, p=p, u_weights=u_weights, v_weights=v_weights)



class transform_to_sphere_fast(nn.Module):
    def __init__(self,):
        super(transform_to_sphere_fast, self).__init__()
...
2023/08/18
"""
class max_spherical_wassersten_distance_fast(nn.Module):
    def __init__(self,num_projections,phi,SSW, phi_op, p=2,max_iter=10,device='cuda'):
        super(max_spherical_wassersten_distance_fast, self).__init__()
    ...
    def forward(self, first_samples,second_samples, train_or_test = "train"):
        ...
        if train_or_test == "train":
            for _ in range(self.max_iter):
                ...
                ssw = self.SSW(first_samples_transform,second_samples_transform, self.num_projections, self.device, p = self.p)
...
if __name__ == "__main__":
    ...
    inputs2 = torch.zeros((32,512, 3)).to(device)

    model = transform_to_sphere_fast()
    model = model.to(device)

    model_op = optim.Adam(model.parameters(), lr=0.01, betas=(0.5, 0.999))

    SSW =  sliced_wasserstein_sphere_fast
    loss = max_spherical_wassersten_distance_fast(num_projections, phi = model, SSW = SSW,  phi_op = model_op, p=2,max_iter=20,device= device)
