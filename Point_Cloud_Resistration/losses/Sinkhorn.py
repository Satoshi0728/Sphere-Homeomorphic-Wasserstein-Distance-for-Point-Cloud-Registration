...
class Sinkhorn_Distance_Loss(torch.nn.Module):
    ...
    def _type_of_cost_norm(type_of_cost_norm = 'L2'):
        if type_of_cost_norm == 'L2':
            p = 2
        elif type_of_cost_norm == 'L1':
            p = 1
        else:
            raise ValueError("type_of_cost_norm must be 'L1' or 'L2'")
        return p