...
#データセットの変形 =============================================================================
class Dataset_Transformation_test:
    ...
    def quaternion_rotate(point_cloud: torch.Tensor, pose_7d: torch.Tensor):
        ...
        if ndim == 2:
            ...
            quat = Dataset_Transformation_test.get_quaternion(pose_7d).expand([N, -1])
            rotated_point_cloud = qrot(quat, point_cloud)

        elif ndim == 3:
            B, N, _ = point_cloud.shape
            quat = Dataset_Transformation_test.get_quaternion(pose_7d).unsqueeze(1).expand([-1, N, -1]).contiguous()
            rotated_point_cloud = qrot(quat, point_cloud)
    ...
    def quaternion_transform(point_cloud: torch.Tensor, pose_7d: torch.Tensor):
        transformed_point_cloud = Dataset_Transformation_test.quaternion_rotate(point_cloud, pose_7d) + Dataset_Transformation_test.get_translation(pose_7d).view(-1, 1, 3).repeat(1, point_cloud.shape[1], 1)      # Ps' = R*Ps + t
...
class Dataset_pytorch(Dataset):
    ...
    def __getitem__(self,index: int):
        source = self.dataset[index].pos
        target = self.dataset[index].y
        rotation = self.igt_rotation_list[index]
        translation = self.igt_translation_list[index]
...
#     # ここで取り出すデータを指定している
#     def __getitem__(self,index: int):
#         print("shape of igt_rotation_list", self.igt_rotation_list[0].shape)
#         source = self.dataset[index].pos
#         target = self.dataset[index].y
#         return target, source
...
if __name__ == "__main__":
    ...
    transformed_target_train_dataset = Dataset_Transformation_test(len(source_train_dataset), angle_range=45, translation_range=0.1)(source_train_dataset)
    #visualize data
