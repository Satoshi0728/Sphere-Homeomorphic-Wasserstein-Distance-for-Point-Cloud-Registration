...
#自作
from data_utils.Data_set_transformation_outlier import Dataset_Transformation_outlier as transform
...
"""
class Data_set_maker_add_noise_and_rigid_transformation_outlier():
    def __init__(self, BASE_DIR = BASE_DIR, dataset = ModelNet, train_or_test = "train" , source_point_num = 2048, target_point_num = 2048, Dataset_Transformation =transform, noise_mean = 0, noise_sigma = 0.03, rigid_transform_angle_range = 45, rigid_transform_translation_range = 1,  outlier_num = 10, outlier_sigma = 0.4):
        super().__init__()
    ...
    def load_dataset(self, dataset, BASE_DIR = BASE_DIR, train_or_test = "train", point_num = 2048):
        ...
        if train_or_test == "train":
            ...
            print("Error! you set long srtings in Data_set_maker_add_noise_and_rigid_transformation_outlier! please set train_or_test as train or test")
            sys.exit(1)
    ...
    def make_dataset_add_pos_and_y(self,source_dataset, target_dataset):
    ...
    def __call__(self):
        ...
        source_dataset_added_noise_rigid_transformed = self.Dataset_Transformation(len(source_dataset_added_noise), angle_range = self.angle_range, translation_range= self.translation_range, outlier_num = self.outlier_num, outlier_sigma = self.outlier_sigma)(source_dataset_added_noise)
        #データセットの結合 posはソースデータセットの座標, yはターゲットデータセットの座標
        dataset = self.make_dataset_add_pos_and_y(source_dataset=source_dataset_added_noise_rigid_transformed, target_dataset= self.ModelNet_target)
        return dataset
