...
from losses import pseudo_max_cos_disimilarity_wassersten_distance
...
def test_one_epoch(device, model, test_loader, criteria, iteration_num, rotation_limit):
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm(test_loader)):
            ...
            loss_val, sphere_template, sphere_source = criteria(template, output['transformed_source'])
...
def main():
    ...
    train_criteria = pseudo_max_cos_disimilarity_wassersten_distance(CSW = CSW, device= device, phi_num = phi_num, lam = lamda_of_regularization,  n_flow_layer =  phi_num_flow_layer, flow_name = flow, mean_or_max_or_softmax = mean_or_max_or_softmax)
