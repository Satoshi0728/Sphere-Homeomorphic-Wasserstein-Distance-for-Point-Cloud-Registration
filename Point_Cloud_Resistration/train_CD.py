...
def main():
   ...
   #データセットの作成=====================================
    ...
    if args.load_model != 'None':
        model ,optimizer, start_epoch = load_checkpoint(model, optimizer, device,filename = args.load_model)
    ...
    #学習=====================================
    train(train_criteria = train_criteria,test_criteria = test_criteria, iteration_num =iteration_num, device = device, optimizer = optimizer, num_epoch = num_epoch, model = model, train_loader = train_dataloader, test_loader = valid_dataloader, writer = writer ,log_dir = log_dir, run_log = run_log, save_point_cloud = save_point_cloud)
