...
from losses import Norm_Flow_structure_optuna
...
class calculate_accuracies():
    ...
    def calculate_accuracy(self, rotation, translation, est_rotation, est_translation, success_ratio_rotation, success_ratio_translation):
        error_temp = []
        # NAN or inf を含む場合は、計算しない
        if np.isinf(rotation).any() or np.isinf(translation).any() or np.isinf(est_rotation).any() or np.isinf(est_translation).any():
            self.inf_check = True
            return np.array([-10000, -10000])
        if np.isnan(rotation).any() or np.isnan(translation).any() or np.isnan(est_rotation).any() or np.isnan(est_translation).any():
            self.nan_check = True
            return np.array([-10000, -10000])
...
def test_one_epoch(device, model, test_loader, criteria, iteration_num, rotation_limit):
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm(test_loader)):
            ...
            # NANチェック
            if calculate_accuracy.inf_check or calculate_accuracy.nan_check:
...
def train(train_criteria, test_criteria, iteration_num, device, optimizer, num_epoch, model, train_loader, test_loader, writer ,log_dir, run_log, save_point_cloud):
    ...
    for epoch in range(num_epoch):
        ...
        #NANが発生したら、学習を終了する
        if test_loss == 1000000:
            run_log.cprint("NAN or INF is occured")
            break
        ...
        run_log.cprint('EPOCH:: %d, Traininig Loss: %f, Testing Loss: %f, Best Loss: %f, Rot error: %f, Best Rot error: %f,  Trans error: %f, Best Trans error: %f, Time: %f'%(epoch+1, train_loss, test_loss, best_test_loss,rot_error, best_rot_error, trans_error, best_trans_error ,tim_fast))


        if best_rot_error >= 100:
            rot_threshold +=1
...
OPTUNA
"""
def objective_variable(device, EPOCH, run_log, iteration_num, train_dataloader, valid_dataloader, writer ,log_dir, save_point_cloud,flow_names):
    def objective(trial):
        feature_model = MLP_Architecture()
        model = PCRNet(feature_model = feature_model)
        train_criteria = chamfer_distance
        model = model.to(device)

        optimizer = get_optimizer(trial, model, run_log)
        rot_error = train(train_criteria = train_criteria,test_criteria = train_criteria, iteration_num =iteration_num, device = device, optimizer = optimizer, num_epoch = EPOCH, model = model, train_loader = train_dataloader, test_loader = valid_dataloader, writer = writer ,log_dir = log_dir, run_log = run_log, save_point_cloud = save_point_cloud)
        return rot_error
    return objective


def get_optimizer(trial, model, run_log):
    weight_decay = trial.suggest_loguniform('weight_decay', 1e-15, 1e-3)
    adam_lr = trial.suggest_loguniform('adam_lr', 1e-7, 1e-1)
    optimizer = torch.optim.Adam(model.parameters(), lr=adam_lr, weight_decay=weight_decay)
    run_log.cprint(f"adam_lr: {adam_lr}, weight_decay: {weight_decay}")
    return optimizer
...
def main():
    ...
    #データセットの作成=====================================
    # pytorch_geometricからmodelnetを持ってきて、ノイズと剛体変換を加えたデータセットを返す。
    ...
    #学習=====================================
    ex_name =  str(experiment_version)

    study = optuna.create_study(
        direction='minimize',
        storage=f"sqlite:///storage/{ex_name}.db",
        study_name=f"{ex_name}",
        load_if_exists=True,
        )
    ...
    while flag:
        try:
            study.optimize(objective_variable(device, EPOCH, run_log, iteration_num, train_dataloader, valid_dataloader, writer ,log_dir, save_point_cloud,flow_names), n_trials=TRIAL_SIZE)
        except KeyboardInterrupt:
            run_log.cprint(f"best_params: {study.best_params}")
            run_log.cprint(f"best_value: {study.best_value}")
            run_log.cprint(f"best_trial: {study.best_trial}")

            user_input = input("キーを入力してください ('q'で終了, 'r'で再開):")
            while (user_input != 'q') and (user_input != 'r'):
