...
def options():
    ...
    parser.add_argument('--cuda_num', type=int, default= 1 , help='cuda_number')
    ...
    parser.add_argument('--angle_r', type=int, default= 0, help='angle_range')
    parser.add_argument('--translation_r', type=int, default= 1, help='translation_range')
    ...
    parser.add_argument('--sinkhorn_eps', type=float, default= 0.1, help='sinkhorn_eps')
...
def main():
    ...
    translation_r = args.translation_r
    #モデル関係
    ...
    #データセットの作成=====================================
    test_loss_list = []
    translation_range = np.arange(0, 4.01, 0.01)
    for translation_limit in translation_range:
        print("translation_limit", translation_limit)
        test_dataset = Data_set_maker_add_noise_and_rigid_transformation(BASE_DIR = BASE_DIR, train_or_test = "test", source_point_num = source_point_num, target_point_num = target_point_num, Dataset_Transformation = transform, noise_mean = mean, noise_sigma = sigma, rigid_transform_angle_range = angle_range, rigid_transform_translation_range = translation_limit)()
        ...
        losses = [translation_limit, CD_loss / len(test_dataset), SD_loss / len(test_dataset), WD_loss / len(test_dataset)]
